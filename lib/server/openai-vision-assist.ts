...
const MAX_VISION_IMAGES = MAX_VISION_PAGES * 2;
...
export function isVisionPageImage(value: unknown): value is VisionPageImage {
  ...
  return typeof page.page === "number" && Number.isInteger(page.page) && page.page > 0
    && (page.region === "full_page" || page.region === "line_crop")
...
export async function requestVisionAssist(
  ...
  const pages = images
    ...
    .filter((image, index, all) => all.findIndex((candidate) => candidate.page === image.page && candidate.region === image.region) === index)
    .slice(0, MAX_VISION_IMAGES);
  const reviewedPages = [...new Set(pages.map((image) => image.page))].slice(0, MAX_VISION_PAGES);
  if (!pages.length) {
    return { ...insufficientResult("No existen imágenes suficientes para una lectura visual auxiliar.", "La visión LLM necesita páginas legibles; el documento original debe conservarse para revisión humana."), model, reviewedPages };
  }
  ...
  const content = [
    ...
    ...pages.flatMap((image) => [
      { type: "input_text", text: `Página ${image.page}; zona ${image.region}.` },
