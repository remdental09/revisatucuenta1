export async function ensureCaseSchema(db: D1Database) {
  await db.batch([
    db.prepare(`CREATE TABLE IF NOT EXISTS cases (id TEXT PRIMARY KEY, patient_name TEXT NOT NULL, contact_email TEXT NOT NULL DEFAULT '', episode_label TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'collecting', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`),
