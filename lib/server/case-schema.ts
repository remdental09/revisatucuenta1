export async function ensureCaseSchema(db: any) {
  await db.batch([
    db.prepare(`CREATE TABLE IF NOT EXISTS cases (id TEXT PRIMARY KEY, owner_user_id TEXT NOT NULL DEFAULT '', owner_email TEXT NOT NULL DEFAULT '', patient_name TEXT NOT NULL, patient_run TEXT NOT NULL DEFAULT '', contact_email TEXT NOT NULL DEFAULT '', episode_label TEXT NOT NULL, retention_mode TEXT NOT NULL DEFAULT 'persistent', status TEXT NOT NULL DEFAULT 'collecting', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`),
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS service_contracts (id TEXT PRIMARY KEY, case_id TEXT NOT NULL UNIQUE, contract_version TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'draft', patient_name TEXT NOT NULL, patient_email TEXT NOT NULL, company_name TEXT NOT NULL, episode_label TEXT NOT NULL, contract_text TEXT NOT NULL, price_clp INTEGER NOT NULL DEFAULT 0, accepted_terms INTEGER NOT NULL DEFAULT 0, data_consent INTEGER NOT NULL DEFAULT 0, mandate_consent INTEGER NOT NULL DEFAULT 0, signer_name TEXT, accepted_at TEXT, payment_status TEXT NOT NULL DEFAULT 'not_started', payment_url TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(case_id) REFERENCES cases(id))`),
  ...
  try { await db.prepare(`ALTER TABLE cases ADD COLUMN patient_run TEXT NOT NULL DEFAULT ''`).run(); } catch { /* Column already exists. */ }
  ...
  try { await db.prepare(`ALTER TABLE cases ADD COLUMN retention_mode TEXT NOT NULL DEFAULT 'persistent'`).run(); } catch { /* Column already exists. */ }
