...
export function PatientPortal() {
  ...
  async function startPatientAnalysis() {
    ...
    const phases = [
      ...
      [72, "Manteniendo el PAM como contexto separado"],
      [88, "Detectando indicios en la cuenta del prestador"],
    ] as const;
...
function PatientAnalysisPanel({ status, progress, stage, notice, pamReady, claimAuthorized, onAuthorizeClaims }: { status: PatientAnalysisStatus; progress: number; stage: string; notice: string; pamReady: boolean; claimAuthorized: boolean; onAuthorizeClaims: () => void }) {
  const phases = [
    ...
    ["Contexto PAM separado", progress >= 72],
...
function isProviderFocusAssessment(assessment: ClinicalAccountAnalysis["lineAssessments"][number]) {
  const text = normalizePortalText(`${assessment.line.description} ${assessment.line.section ?? ""}`);
  return !/(bonificacion|bonificado|copago|liquidacion|rechazo|pam|ajuste|diferencia administrativa)/.test(text);
}

function providerFocusAssessmentCount(analysis: ClinicalAccountAnalysis) {
  return analysis.lineAssessments.filter(isProviderFocusAssessment).length;
}
...
function DeveloperOverview({ total, pamReady, analysis, claimAuthorized }: { total: number; pamReady: boolean; analysis?: ClinicalAccountAnalysis; claimAuthorized: boolean }) {
  ...
  const focusCount = analysis ? providerFocusAssessmentCount(analysis) : 0;
  return <div className="developer-overview"><div className="dev-flow-card"><div className="card-heading"><div><span className="card-kicker">FLUJO DEL EXPEDIENTE</span><h3>Cuenta clínica primero</h3></div><span className="dev-percentage">{progress}%</span></div><div className="dev-flow"><FlowStep number="01" title="Cuenta" state="complete" detail="Recibida"/><i/><FlowStep number="02" title="Desfragmentación" state={analysisReady ? "complete" : "current"} detail={analysisReady ? "Hipótesis listas" : "En curso"}/><i/><FlowStep number="03" title="Contexto PAM" state={pamReady ? "complete" : "pending"} detail={pamReady ? "Separado" : "Opcional"}/><i/><FlowStep number="04" title="Preinforme" state="pending" detail="Pendiente"/></div><div className="dev-progress"><i style={{ width: `${progress}%` }} /></div></div><div className="developer-scope-card"><div><span className="card-kicker">ALCANCE ACTUAL</span><h3>Posibles desfragmentaciones del prestador</h3><p>Se revisan glosas, códigos, cantidades y vínculos dentro de la cuenta clínica. La ausencia de bonificación en el PAM se conserva como dato documental y no se interpreta como cobro indebido.</p></div><span>FASE 1</span></div><div className="developer-callout"><span>!</span><div><b>{analysisReady && claimAuthorized ? "Autorización registrada" : analysisReady ? "Hipótesis de cuenta clínica disponibles" : pamReady ? "Cuenta clínica lista; PAM sólo como contexto" : "El caso tiene información pendiente"}</b><p>{analysisReady && claimAuthorized ? "El paciente autorizó a RevisaTuCuenta a preparar y presentar los reclamos pertinentes ante el prestador, dentro del alcance informado." : analysisReady ? "El motor conserva cada línea, probabilidad, página y evidencia faltante para revisar una posible desfragmentación del prestador. La bonificación no decide este resultado." : pamReady ? "La cuenta clínica puede analizarse de forma independiente. El PAM está disponible como contexto, sin convertir una no bonificación en una irregularidad." : "La cuenta clínica puede analizarse de forma independiente. El PAM podrá incorporarse después como contexto documental."}</p><button>Revisar documentos faltantes →</button></div></div><div className="dev-analysis-grid"><article><span className="card-kicker">CUENTA CLÍNICA</span><strong>{money(total)}</strong><small>Total informado por el prestador</small></article><article><span className="card-kicker">DESFRAGMENTACIÓN</span><strong>{analysisReady ? `${focusCount} líneas` : "Pendiente"}</strong><small>{analysisReady ? "Líneas dentro del foco del prestador" : "Pulsa Siguiente en vista paciente"}</small></article><article><span className="card-kicker">PRÓXIMA ACCIÓN</span><strong>{analysisReady ? "Contrastar" : "Clasificar"}</strong><small>{analysisReady ? "Código, glosa, contrato y respaldo de uso" : "Ordenar la cuenta clínica"}</small></article></div><div className="developer-actions"><button className="portal-button portal-button-primary">Abrir analizador <span>→</span></button><button className="portal-button portal-button-secondary">Exportar preinforme</button></div>{analysisReady && <DeveloperAnalysisDetail analysis={analysis!} claimAuthorized={claimAuthorized} />}</div>;
}
...
function DeveloperTraceability({ account, analysis, claimAuthorized }: { account: string; analysis?: ClinicalAccountAnalysis; claimAuthorized: boolean }) {
  ...
  return <div className="traceability-view"><div className="traceability-toolbar"><div><span className="card-kicker">MATRIZ DE CUENTA CLÍNICA</span><h3>Línea de cuenta → prestación presumible</h3></div><button className="portal-button portal-button-secondary">Exportar matriz</button></div><div className="trace-table"><div className="trace-head"><span>Línea de cuenta</span><span>Prestación presumible</span><span>Indicio en cuenta</span><span>Valor</span></div>{rows.map((row) => <div className="trace-row" key={row.name}><div><b>{row.name}</b><small>Cargo individual · cuenta del prestador</small></div><span>{row.anchor}</span><em className={row.state === "Identificado" ? "identified" : "review"}>{row.state}</em><strong>{row.amount ? money(row.amount) : "Por desglosar"}</strong></div>)}</div><div className="trace-note"><span>i</span><p>Esta matriz busca posibles desfragmentaciones dentro de la cuenta clínica. El PAM no se usa aquí para concluir improcedencia: una prestación no bonificada sólo requiere aclarar su tratamiento documental.</p></div>{analysis && <DeveloperAnalysisDetail analysis={analysis} claimAuthorized={claimAuthorized}/>}</div>;
}
...
function DeveloperAnalysisDetail({ analysis, claimAuthorized }: { analysis: ClinicalAccountAnalysis; claimAuthorized?: boolean }) {
  ...
  const providerAssessments = assessments.filter(isProviderFocusAssessment);
  const contextAssessments = assessments.filter((assessment) => !isProviderFocusAssessment(assessment));
  const candidates = providerAssessments.filter((item) => item.candidates.length > 0);
  const candidateAmount = candidates.reduce((sum, item) => sum + item.line.amount, 0);
  const providerLineIds = new Set(providerAssessments.map((assessment) => assessment.line.id));
  const providerAnomalies = analysis.anomalies.filter((anomaly) =>
    anomaly.type !== "opaque_adjustment" || anomaly.lineIds.some((lineId) => providerLineIds.has(lineId)),
  );
  ...
  return <section className="developer-analysis-detail"><div className="developer-analysis-detail-head"><div><span className="card-kicker">DESFRAGMENTACIÓN DEL PRESTADOR</span><h3>Evidencia línea por línea</h3><p>Hipótesis técnicas sobre la cuenta clínica; no son conclusiones jurídicas ni montos recuperables.</p></div><div className="developer-analysis-badges"><span>{providerAssessments.length} líneas en foco</span><span className={claimAuthorized ? "authorized" : "pending"}>{claimAuthorized ? "Autorización otorgada" : "Autorización pendiente"}</span></div></div><div className="developer-detail-metrics"><article><b>{providerAssessments.length}</b><small>Líneas de cuenta clínica en foco</small></article><article><b>{candidates.length}</b><small>Con hipótesis de desfragmentación</small></article><article><b>{money(candidateAmount)}</b><small>Valor bajo hipótesis del prestador</small></article><article><b>{providerAnomalies.length}</b><small>Señales de cuenta para contrastar</small></article></div><div className="developer-line-table"><div className="developer-line-head"><span>Línea / origen</span><span>Hipótesis de inclusión</span><span>Valor</span></div>{providerAssessments.map((assessment) => { const candidate = [...assessment.candidates].sort((left, right) => right.probability - left.probability)[0]; return <article key={assessment.line.id}><div><b>{assessment.line.description}</b><small>{assessment.line.section || "Sección no identificada"} · pág. {assessment.line.page}{assessment.line.code ? ` · código ${assessment.line.code}` : ""}</small></div><div>{candidate ? <><strong>{labelForBundle(candidate.bundle)} · {Math.round(candidate.probability * 100)}%</strong><small>{candidate.reasons[0] || "Coincidencia contextual"}{candidate.missingEvidence.length ? ` · Falta: ${candidate.missingEvidence.join("; ")}` : ""}</small></> : <><strong>Sin hipótesis suficiente</strong><small>Requiere clasificación documental adicional.</small></>}</div><b>{money(assessment.line.amount)}</b></article>; })}</div>{contextAssessments.length > 0 && <div className="developer-context-detail"><b>Fuera del foco de desfragmentación</b><p>Estas líneas se conservan como contexto de la cuenta. Que aparezcan como no bonificadas o agrupadas administrativamente no permite concluir que estén mal cobradas.</p>{contextAssessments.map((assessment) => <span key={assessment.line.id}>{assessment.line.description} · {money(assessment.line.amount)} · pág. {assessment.line.page}</span>)}</div>}{!!providerAnomalies.length && <div className="developer-anomaly-detail"><div className="developer-analysis-detail-head"><div><b>Señales de cuenta</b><p>Indicadores que deben contrastarse con contrato, registro de uso y respuesta del prestador.</p></div></div>{providerAnomalies.map((anomaly, index) => <article key={`${anomaly.type}-${index}`}><span>{anomaly.severity}</span><div><b>{anomaly.type.replaceAll("_", " ")}</b><p>{anomaly.explanation}</p><small>{anomaly.lineIds.join(" · ")}</small></div></article>)}</div>}<p className="developer-detail-foot">Corpus observado: {analysis.observedCorpus.caseCount} casos · {analysis.observedCorpus.observationCount} observaciones · {analysis.observedCorpus.patternCount} patrones. El PAM y la bonificación quedan fuera de este foco inicial. {analysis.observedCorpus.learningBoundary}</p></section>;
}
