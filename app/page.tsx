...
function ResultStep({ docs, availability, onContinue }: { docs: UploadedDoc[]; availability: { cuenta: boolean; pam: boolean; contrato: boolean }; onContinue: () => void }) {
  ...
  return <div className="stageInner"><StageTitle eyebrow="PASO 6 DE 7" title="Tu primer informe está disponible" copy="Este informe cuantifica la cuenta y contrasta lo que ya aparece en los PAM. No afirma por sí solo que exista un cobro indebido."/><div className="resultHero"><div><span className="statusPill">PRIMER INFORME COMPLETADO</span><h2>{isEmblematic ? "$6.912.876 revisados con sus liquidaciones" : "La cuenta ya puede comenzar a revisarse"}</h2><p>{isEmblematic ? "El total de los PAM queda a $1 del total de la cuenta; además identificamos montos bonificados y a cargo del paciente." : availability.pam ? "También encontramos PAM para preparar el cruce de cobros y bonificaciones." : "Puedes agregar el PAM más adelante para contrastar las bonificaciones."}</p></div><div className="score"><strong>{availability.pam ? 78 : 62}</strong><span>/100</span><small>Calidad documental</small></div></div><div className="metricGrid"><article><span>{isEmblematic ? "$6,91 M" : docs.length}</span><b>{isEmblematic ? "Total cuenta clínica" : "Documentos procesados"}</b><small>{isEmblematic ? "Fuente: página 8" : `${docs.length} archivo${docs.length === 1 ? "" : "s"}`}</small></article><article><span>{isEmblematic ? "$6,46 M" : typeCount}</span><b>{isEmblematic ? "Bonificación informada" : "Tipos identificados"}</b><small>{isEmblematic ? "93,46% · PAM págs. 20–22" : availability.pam ? "Cuenta + PAM" : "Cuenta clínica"}</small></article><article><span>{isEmblematic ? "$452.175" : availability.pam ? 4 : 2}</span><b>{isEmblematic ? "A cargo del paciente" : "Puntos a revisar"}</b><small>{isEmblematic ? "6,54% · PAM págs. 11 y 20–22" : "Sin conclusiones definitivas"}</small></article></div><div className="findings"><h3>Qué dice este primer informe</h3><div><span className="findingIcon blue">↔</span><p><b>{isEmblematic ? "La cuenta y los PAM prácticamente concilian" : "Documentación conectada"}</b><small>{isEmblematic ? "$6.912.876 en la cuenta versus $6.912.875 en los PAM: diferencia de $1 para confirmar." : "La evidencia quedó vinculada a su archivo original."}</small></p><button>Fuente</button></div><div><span className="findingIcon amber">!</span><p><b>{isEmblematic ? "Hay prestaciones sin cobertura que requieren explicación" : "Puntos que requieren revisión"}</b><small>{isEmblematic ? "Los PAM registran $440.069 de copago en clínica y $12.106 en consulta de urgencia." : "El informe separa hechos documentales de hipótesis."}</small></p><button>Fuente</button></div>{!availability.contrato && <div><span className="findingIcon gray">+</span><p><b>El contrato no bloquea este resultado</b><small>Se solicitará después para evaluar si los motivos de no cobertura respetan el plan, sus topes y exclusiones.</small></p><button>Etapa posterior</button></div>}</div><div className="stageActions end"><button className="button primary" onClick={onContinue}>Ver informe completo →</button></div></div>;
}
...
function Dashboard({ docs, caseId, availability }: { docs: UploadedDoc[]; caseId: string; availability: { cuenta: boolean; pam: boolean; contrato: boolean } }) {
  ...
  const isEmblematic = docs.some(d => d.segments?.some(s => s.pages === "Páginas 1–8"));
  return <div className="dashboard"><div className="dashboardHead"><div><p>CASO {caseId.slice(0,8).toUpperCase()}</p><h1>Hospitalización por apendicitis</h1><span>Clínica INDISA · Nueva Masvida</span></div><button className="button ghost">＋ Agregar documento</button></div><div className="dashboardGrid"><section className="casePanel overview"><div className="panelTitle"><h2>Estado del expediente</h2><span className="statusPill">PRIMER INFORME LISTO</span></div><div className="routeMap"><div className="complete"><span>✓</span><b>Documentos</b><small>{docs.length} archivo{docs.length === 1 ? "" : "s"}</small></div><i/><div className="complete"><span>✓</span><b>Primer informe</b><small>Disponible</small></div><i/><div className="current"><span>3</span><b>Cruces</b><small>{availability.pam ? "Cuenta ↔ PAM" : "Espera PAM"}</small></div><i/><div><span>4</span><b>Reclamo</b><small>Pendiente</small></div></div></section><section className="casePanel nextAction"><p>INFORME DISPONIBLE</p><h2>{isEmblematic ? "$452.175 a cargo del paciente" : "Revisa tu primer informe"}</h2><span>{isEmblematic ? "La cuenta y los PAM casi concilian: existe una diferencia documental de $1 que quedó señalada." : "No necesitas contrato para abrirlo. El contrato se usará después para calcular coberturas y topes."}</span><button className="button primary" onClick={() => setReportOpen(true)}>Abrir primer informe →</button></section><section className="casePanel evidence"><div className="panelTitle"><h2>Documentos y evidencia</h2><button>Ver todo</button></div>{docs.map(d=><div className="evidenceRow" key={d.id}><span className="miniPdf">PDF</span><p><b>{d.name}</b><small>{d.size} · Clasificado</small></p><em>✓</em></div>)}</section><section className="casePanel pendingEngines"><div className="panelTitle"><h2>Motores del caso</h2><span>Avance por etapas</span></div>{["Primer informe documental","Reconstrucción de cuenta","Cruce Cuenta ↔ PAM","Reglas de contrato","Hallazgos y evidencia","Generador de reclamos"].map((x,i)=><div key={x}><span>{i < 3 && isEmblematic ? "✓" : i === 0 ? "✓" : i < 3 ? "↗" : "○"}</span><b>{x}</b><small>{i < 3 && isEmblematic ? "Completado preliminar" : i === 0 ? "Completado" : i < 3 ? "Listo para continuar" : "Etapa posterior"}</small></div>)}</section></div><div className="disclaimer"><b>Importante:</b> RevisaTuCuenta organiza información y genera hipótesis de revisión. No reemplaza una auditoría médica, asesoría legal ni garantiza una devolución.</div>{reportOpen && <PreliminaryReport isEmblematic={isEmblematic} availability={availability} onClose={() => setReportOpen(false)}/>}</div>;
}

function PreliminaryReport({ isEmblematic, availability, onClose }: { isEmblematic: boolean; availability: { cuenta: boolean; pam: boolean; contrato: boolean }; onClose: () => void }) {
  return <div className="reportOverlay" role="dialog" aria-modal="true" aria-labelledby="report-title"><button className="reportBackdrop" aria-label="Cerrar informe" onClick={onClose}/><article className="reportSheet"><header><div><p>REVISATUCUENTA · INFORME PRELIMINAR</p><h2 id="report-title">Primer informe del caso</h2><span>Hospitalización por apendicitis · datos personales omitidos</span></div><button onClick={onClose} aria-label="Cerrar">×</button></header><div className="reportStatus"><span>✓</span><p><b>Informe útil generado sin contrato</b><small>Describe montos y diferencias documentales. El contrato se usará después para juzgar coberturas, topes y exclusiones.</small></p></div>{isEmblematic ? <><section><h3>1. Resumen financiero</h3><div className="reportMetrics"><article><span>Cuenta clínica</span><strong>$6.912.876</strong><small>Fuente · pág. 8</small></article><article><span>Bonificación PAM</span><strong>$6.460.700</strong><small>93,46% · págs. 11, 20–22</small></article><article><span>A cargo del paciente</span><strong>$452.175</strong><small>6,54% · págs. 11, 20–22</small></article></div><div className="reconciliation"><div><span>Cuenta clínica</span><b>$6.912.876</b></div><i>versus</i><div><span>Suma valores PAM</span><b>$6.912.875</b></div><strong>Diferencia: $1</strong></div><p className="reportExplanation">Los dos PAM cubren el mismo episodio: $6.852.344 del folio principal y $60.531 de la consulta de urgencia. La suma queda a un peso del total de la cuenta; esto parece una diferencia de cuadratura documental y debe confirmarse, no una señal de sobrecobro por sí sola.</p></section><section><h3>2. Composición relevante de la cuenta</h3><div className="financialTable"><div className="tableHead"><span>Rubro</span><span>Monto</span><span>Origen</span></div><div><b>Pabellón · apendicectomía</b><span>$1.914.834</span><em>Pág. 2</em></div><div><b>Honorarios equipo médico</b><span>$1.485.096</span><em>Pág. 8</em></div><div><b>Insumos de pabellón</b><span>$1.190.610</span><em>Págs. 1–2</em></div><div><b>Medicamentos de pabellón</b><span>$567.100</span><em>Pág. 2</em></div><div><b>Día cama individual</b><span>$452.075</span><em>Pág. 1</em></div><div><b>TAC abdomen y pelvis</b><span>$408.981</span><em>Pág. 7</em></div></div></section><section><h3>3. Hallazgos preliminares</h3><div className="reportFindings"><article><span>01</span><div><b>Los PAM explican quién financia casi todo el total.</b><p>Bonificación informada de $6.460.700 y copago total de $452.175. El informe ya puede mostrar este reparto aunque falte el contrato.</p><small>Fuente · PAM, págs. 11 y 20–22</small></div></article><article><span>02</span><div><b>Hay $440.069 de copago en el PAM principal.</b><p>Incluye medicamentos, materiales y gastos marcados como “no cubiertos por el plan”, más diferencias de bonificación. Debe revisarse cada motivo antes de reclamar.</p><small>Fuente · PAM, págs. 20–22</small></div></article><article><span>03</span><div><b>La liquidación identifica consumos específicos sin cobertura.</b><p>Entre ellos aparecen artículos de uso del paciente y prestaciones como instalación de vía venosa y fleboclisis. Que figuren sin cobertura no basta para afirmar que el cobro sea improcedente.</p><small>Fuente · liquidación, pág. 18</small></div></article><article><span>04</span><div><b>El episodio está fragmentado entre dos prestadores y dos PAM.</b><p>La clínica concentra hospitalización, pabellón, medicamentos y materiales; otra sociedad concentra consulta, honorarios, imagenología y otros servicios. El cruce debe conservar esa separación.</p><small>Fuente · cuenta, págs. 1–8; PAM, págs. 9–22</small></div></article></div></section><section><h3>4. Qué falta para una conclusión de cobertura</h3><p>El contrato no es necesario para conocer los montos ni detectar diferencias. Sí se necesita para evaluar si cada copago, exclusión o prestación no arancelada fue aplicada conforme al plan. Mientras no esté disponible, estos puntos son preguntas fundadas y no acusaciones.</p><div className="nextQuestions"><b>Próximas verificaciones</b><ul><li>Confirmar la diferencia de $1 entre cuenta y PAM.</li><li>Solicitar el fundamento de cada gasto marcado “no cubierto por el plan”.</li><li>Revisar si los insumos cobrados separadamente están incluidos en día cama o derecho de pabellón según el convenio aplicable.</li><li>Incorporar el contrato para contrastar porcentajes, topes y exclusiones.</li></ul></div></section></> : <><section><h3>1. Documentación identificada</h3><div className="reportRows"><div><b>Cuenta clínica</b><span>Disponible</span><em>Verificada</em></div><div><b>PAM / liquidaciones</b><span>{availability.pam ? "Disponible" : "No agregado"}</span><em className={availability.pam ? "" : "pending"}>{availability.pam ? "Verificado" : "Opcional ahora"}</em></div><div><b>Contrato de salud</b><span>{availability.contrato ? "Disponible" : "No agregado"}</span><em className={availability.contrato ? "" : "pending"}>{availability.contrato ? "Verificado" : "Etapa posterior"}</em></div></div></section><section><h3>2. Alcance actual</h3><p>La cuenta ya permite ordenar prestaciones, rubros y montos. Cuando agregues el PAM podremos cuantificar bonificación y copago; el contrato quedará reservado para evaluar las reglas de cobertura.</p></section></>}<footer><span>Informe preliminar · No constituye auditoría médica ni asesoría legal.</span><button className="button primary" onClick={onClose}>Volver al caso</button></footer></article></div>;
}
