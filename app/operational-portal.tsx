...
import { analyzePamStandalone, type ClinicalAccountAnalysis, type ChileanBillingLine, type PamStandaloneAnalysis, type PamTraceability } from "../lib/rules/chilean-account";
import type { FunctionalEquivalenceAlert } from "../lib/rules/observed-corpus";
import {
  buildPatientResult,
  bestCombinedCandidate,
  DISPUTE_LLM_THRESHOLD as LLM_CANDIDATE_THRESHOLD,
  llmCandidateForLine,
  possibleDisputeAmount,
  type PatientResult,
} from "../lib/rules/patient-result";
...
function AuthenticationLoading() {
  return <main className="patient-login"><section className="patient-login-card"><PortalBrand/><h1>Verificando acceso…</h1><p>Estamos protegiendo el acceso a tu revisión.</p></section></main>;
}
...
function standalonePam(document?: CaseDocument): PamStandaloneAnalysis | undefined {
  const pam = document?.extraction?.pam;
  if (!pam) return undefined;
  const lines: ChileanBillingLine[] = pam.lines.map((line, index) => ({
    ...line,
    id: `${document.id}-${index}`,
    documentId: document.id,
  }));
  return analyzePamStandalone(lines, pam.fields);
}
...
function PatientStart({ userEmail, onCreated }: { userEmail: string; onCreated: (caseId: string, extraction?: DocumentExtraction) => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const created = await fetch("/api/cases", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id, patientName: name.trim(), episodeLabel: episode }) });
...
function PatientSummary({ account, pam, pamStandalone, reviewAmount, analysisAvailable, analysisRunning, progress, stage, contract, busy, readerReviewRequired, readerChangeNeeded, onAccount, onPam, onContractDocument, onAnalyze, onOpenContract, contractBusy }: { account?: CaseDocument; pam?: CaseDocument; pamStandalone?: PamStandaloneAnalysis; reviewAmount: number; analysisAvailable: boolean; analysisRunning: boolean; progress: number; stage: string; contract?: ServiceContract; busy: boolean; readerReviewRequired: boolean; readerChangeNeeded: boolean; onAccount: () => void; onPam: () => void; onContractDocument: () => void; onAnalyze: () => void; onOpenContract: () => void; contractBusy: boolean }) {
  ...
  return <>
    <section className="patient-card patient-review-status-card patient-space-review-card">
      ...
      {pamStandalone && <PatientPamStandalonePanel analysis={pamStandalone} onPam={onPam} busy={busy} />}
    ...
    <section className="patient-card next-card"><span className="card-kicker">SIGUIENTE PASO</span><h2>{analysisAvailable ? hasIrregularities ? "Revisa el total en disputa" : "Resultado de la revisión" : accountReceived ? "Obtén el resultado de tu cuenta" : pamReceived ? "Falta la cuenta clínica" : "Completa tus documentos"}</h2><p>{analysisAvailable ? hasIrregularities ? "Te mostramos el total preliminar que conviene revisar. El detalle técnico queda reservado para el equipo revisor." : "Con la información disponible no encontramos un monto que requiera revisión." : documentsReceived ? "Carga la cuenta clínica para obtener el resultado y el total preliminar de la revisión." : "Carga la cuenta clínica para obtener el resultado de la revisión."}</p></section>
  </>;
}

function PatientPamStandalonePanel({ analysis, onPam, busy }: { analysis: PamStandaloneAnalysis; onPam: () => void; busy: boolean }) {
  const coverageLabel = analysis.coverageStatus === "covered" ? "Cobertura completa informada" : analysis.coverageStatus === "partial" ? "Cobertura parcial o copago" : analysis.coverageStatus === "not_covered" ? "Sin bonificación suficiente" : "Cobertura no desglosada";
  return <section className="patient-pam-standalone" aria-label="Análisis propio del PAM">
    <div className="patient-pam-standalone-head"><div><span className="card-kicker">LECTURA PROPIA DEL PAM</span><h3>Qué informa tu PAM</h3><p>{analysis.summary}</p></div><span className={`patient-pam-standalone-status ${analysis.status}`}>{coverageLabel}</span></div>
    <div className="patient-pam-standalone-metrics"><div><b>{analysis.lines.length}</b><small>Prestaciones detectadas</small></div><div><b>{analysis.identifiedLineCount}</b><small>Con glosa identificable</small></div><div><b>{analysis.genericLineCount}</b><small>Glosas generales</small></div><div><b>{analysis.copayTotal === null ? "—" : money(analysis.copayTotal)}</b><small>Copago informado</small></div></div>
    <div className="patient-pam-standalone-findings">{analysis.findings.slice(0, 4).map((finding) => <article key={finding.id}><b>{finding.title}</b><small>{finding.explanation}</small></article>)}</div>
    <p className="patient-pam-standalone-note">Esta lectura describe sólo el PAM. Todavía no compara ni determina desfragmentaciones de la cuenta clínica.</p>
    <button className="portal-button portal-button-secondary" onClick={onPam} disabled={busy}>Reemplazar PAM</button>
  </section>;
}
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/cases", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id, patientName: patientName.trim() || "Paciente", episodeLabel: episodeLabel.trim() || "Revisión de cuenta clínica" }),
      });
      const payload = await response.json().catch(() => ({}));
  ...
  async function onFile(file: File, classification: string) {
    ...
    try {
      ...
      let result = previousAccount
        ? await replaceAccountDocument(selected, file, previousAccount.id, updateProgress)
...
function DeveloperTraceability({ snapshot, onExport, onExportMarkdown }: { snapshot: Snapshot; onExport: () => void; onExportMarkdown: () => void }) { const pamAnalysis = standalonePam(pamDoc(snapshot)); return <div className="traceability-view"><DeveloperCaseIdentity snapshot={snapshot}/><div className="traceability-toolbar"><div><span className="card-kicker">MATRIZ DE CUENTA CLÍNICA</span><h3>Evidencia línea por línea</h3></div><div className="traceability-toolbar-actions"><button className="portal-button portal-button-secondary" onClick={onExport}>Exportar .json</button><button className="portal-button portal-button-secondary" onClick={onExportMarkdown}>Exportar .md</button></div></div>{pamAnalysis && <DeveloperPamStandalonePanel analysis={pamAnalysis}/>} {snapshot.analysis ? <DeveloperAnalysisDetail analysis={snapshot.analysis}/> : <section className="trace-note"><span>i</span><p>Ejecuta el análisis desde Resumen para generar la matriz.</p></section>}</div>; }
...
function DeveloperPamStandalonePanel({ analysis }: { analysis: PamStandaloneAnalysis }) {
  const coverageLabel = analysis.coverageStatus === "covered" ? "Cobertura informada completa" : analysis.coverageStatus === "partial" ? "Cobertura parcial / copago" : analysis.coverageStatus === "not_covered" ? "Sin bonificación suficiente" : "Cobertura no desglosada";
  return <section className="developer-pam-standalone"><div className="precedent-projection-head"><div><span className="card-kicker">ANÁLISIS PROPIO DEL PAM · PAM-V1</span><h3>{coverageLabel}</h3><p>{analysis.summary}</p></div><div className="developer-analysis-badges"><span>{analysis.lines.length} prestaciones</span><span>{analysis.genericLineCount} glosas generales</span></div></div><div className="developer-detail-metrics"><article><b>{analysis.billedTotal === null ? "—" : money(analysis.billedTotal)}</b><small>Total informado</small></article><article><b>{analysis.bonusTotal === null ? "—" : money(analysis.bonusTotal)}</b><small>Bonificación informada</small></article><article><b>{analysis.copayTotal === null ? "—" : money(analysis.copayTotal)}</b><small>Copago informado</small></article><article><b>{analysis.identifiedLineCount}</b><small>Glosas identificadas</small></article></div><div className="pam-standalone-findings">{analysis.findings.map((finding) => <article key={finding.id}><div><span className={`functional-alert-level ${finding.severity === "high" ? "high" : finding.severity === "review" ? "medium" : "context"}`}>{finding.severity === "high" ? "Prioritaria" : finding.severity === "review" ? "Revisar" : "Informativa"}</span><b>{finding.title}</b></div><p>{finding.explanation}</p><small><strong>Falta:</strong> {finding.missingEvidence.join(" · ")}</small></article>)}</div><div className="functional-equivalence-summary"><strong>Alcance de esta etapa</strong><p>El PAM se describe por separado. Aquí no se concilia con la cuenta clínica ni se calculan desfragmentaciones; esas tareas quedan para una fase posterior con cuenta, contrato, plan y arancel.</p></div></section>;
}
