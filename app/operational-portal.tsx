...
function DeveloperDocuments({ snapshot, busy, onFile, onAnalyze }: { snapshot: Snapshot; busy: boolean; onFile: (file: File, classification: string) => void; onAnalyze: () => void }) { return <div className="developer-documents"><div className="traceability-toolbar"><div><span className="card-kicker">DOCUMENTOS DEL CASO</span><h3>Fuentes cargadas</h3></div><span className="document-replacement-note">Los archivos nuevos quedan vinculados al caso</span></div><div className="dev-document-grid"><OperationalDoc type="Cuenta clínica" document={accountDoc(snapshot)} classification="Cuenta clínica" busy={busy} onFile={onFile} analysisAvailable={Boolean(snapshot.analysis)} onAnalyze={onAnalyze}/><OperationalDoc type="PAM / liquidación" document={pamDoc(snapshot)} classification="PAM / liquidación" busy={busy} onFile={onFile}/><OperationalDoc type="Contrato / plan" document={snapshot.documents.find((doc) => /contrato|plan/i.test(doc.classification))} classification="Contrato" busy={busy} onFile={onFile}/></div></div>; }
